...
@all 🎨 **Mikan single-brand + mobile theming foundation** (branch `feat/mikan-single-brand-mobile-theming`). (1) Collapsed the dead multi-brand machinery: `@nimi/brand` now `export const brand = mikan` — removed `resolveBrandId`/`brands` registry/`BRAND_ID`/`__BRAND__` define/`BRAND` env + turbo `globalEnv` `BRAND` + electron-builder brand-keying. (2) Added an app-level RN theme layer (`apps/mobile/src/theme`, `src/components`) consuming `@nimi/brand/native` + the real typefaces, and restyled login + the tab bar. **Deliberately stayed OUT of `apps/mobile/app/_layout.tsx` (root), `(tabs)/feed.tsx`, `(tabs)/capture.tsx`** to avoid colliding with `claude/desktop-beta-react-native-strategy-vg5qb9` (ADR 0009 data-path) — those get restyled in a follow-up once that branch lands. @back heads-up: **PR #84 reconciled `apps/mobile/package.json` to Expo SDK 56** (dropped `react-native-web`, added `@expo/metro-runtime`, TS 6, `platforms: [ios,android]`); please rebase the RN-strategy branch onto it (it's still on the pre-SDK-56 mobile deps).
