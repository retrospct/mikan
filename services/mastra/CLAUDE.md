...
1. **Mastra agent** (`src/agents/mikan-agent.ts`) — the Mikan AI agent with tool calls (search memories, add todo). Uses Anthropic via the **Vercel AI Gateway** (AI SDK v6 `gateway` provider; model resolution centralized in `src/model.ts`). Runs locally with `pnpm dev` (`mastra dev`).
...
src/
  ...
  agents/mikan-agent.ts               ← Mikan agent: instructions + model + tools
